...
auto parse_window_size(gsl::span<char const* const> args) -> math::vector2i {
    ...
    if(arg_end == args[0] || x <= 0) {
        throw std::invalid_argument("First argument to '--window_size' was not a positive integer: "s + args[0]);
    }
    int const y = std::strtol(args[1], &arg_end, 10);
    if(arg_end == args[1] || x <= 0) {
        throw std::invalid_argument("Second argument to '--window_size' was not a positive integer: "s + args[1]);
    }
